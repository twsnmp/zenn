...
## 定期レポートにAIサマリーを追加
オンにすると、LLMを利用してその日のネットワーク状態の要約（AIサマリー）を作成し、レポートに含めます。これにはマップ設定でのLLM設定が必要です。
...
## Webhook通知
汎用的なWebhookによる通知も可能です。通知先のURLを設定することで、障害発生時にJSON形式のデータを送信します。

## 通知スケジュール
各通知設定に対して、別に作成した「通知スケジュール」を割り当てることができます。これにより、深夜や休日のみ特定の通知を無効化するといった柔軟な運用が可能です。
