...
## LLM(ChatGPTなど)の設定
AI解説機能やサマリー機能を利用するためのLLMの設定です。

- **LLM種別**: OpenAI, Anthropic, Google Geminiなどが選択可能です。
- **URL**: APIのエンドポイントURLを指定します。
- **APIキー**: 各AIサービスから取得した秘密鍵を入力します（パスワード属性で隠蔽されます）。
- **モデル**: 使用するAIモデル名（例：gpt-4, gemini-pro）を指定します。

## マップ表示のカスタマイズ
マップの見た目や操作性に関する詳細な設定です。

- **ノードにIPアドレスを表示**: オンにすると、マップ上の各ノードのアイコンの下にIPアドレスを表示します。
- **選択の透過度**: アイコン選択時の枠の透過度を調整できます。
- **ノードの背景ボックスを削除**: ノード名の背後にあるボックスを表示しないように設定できます。
...
# 描画アイテムの管理
マップ上の図形やテキストなどの描画アイテムに対して、以下の高度な操作が可能です。

- **グループ化**: 複数のアイテムをグループとしてまとめ、一括で移動させることができます。
- **編集ロック**: アイコンの位置や設定を固定し、誤操作による移動や削除を防ぎます。
- **コピー＆ペースト**: 既存の描画アイテムの設定をコピーして、新しいアイテムを素早く作成できます。
- **複数移動**: シフトキーを押しながら選択することで、複数のノードやアイテムを同時に移動させることができます。
