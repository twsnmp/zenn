...
# LLM（大規模言語モデル）連携
TWSNMP FCは、OpenAIなどのLLMと連携して、ログの解説や分析を支援する機能を備えています。

![](/images/books/twsnmpfc-manual/llm_ai.png)

### ログ・トラップのAI解説
SyslogやSNMP TRAPの画面で、特定のログを選択して「AI解説」ボタンをクリックすると、LLMがそのログの意味や推奨される対応策を日本語で分かりやすく解説してくれます。
専門的な知識が必要なログの内容を迅速に理解するのに役立ちます。

### レポートのAIサマリー
定期レポートの送信設定において、AIによるサマリー（要約）機能を有効にすると、その日のネットワークやセキュリティの状態をLLMが分析し、人間が読みやすい要約文を添えて通知してくれます。

### MIB検索支援
MIBブラウザで、特定のオブジェクトの意味が分からない場合に、LLMに対して質問したり、関連するMIBを検索させたりすることができます。
