...
SFTPおよびFTP/FTPSプロトコルを利用して、リモートサーバーから安全にログファイルをダウンロードして読み込みます。

### HTTP / Web APIからの取得 (v2.1.0)

REST APIやWebサーバーからHTTP/HTTPS GET経由でログを取得できます。ヘッダー認証（BearerトークンやAPIキー）にも対応しています。

### Grafana Loki連携 (v2.1.0)

Grafana LokiのLogQLエンドポイントに接続し、指定したストリームセレクターやクエリ条件でログを取得・インポートします。

### Elasticsearch連携 (v2.1.0)

ElasticsearchまたはOpenSearchクラスタのREST APIからインデックス指定でログを高速に取得できます。

### アーカイブ内のファイル名パターン

単一ファイルやSCP/SFTPでZIPやtar.gz等の圧縮アーカイブを指定した場合、「アーカイブ内ファイル名パターン」（例: `*.log`, `syslog*`）を設定することで、アーカイブ内部の対象ファイルだけを自動展開して読み込めます。

# ログ保存先（ストレージエンジン）の選択 (v2.1.0)

ログ分析設定画面では、データの保存形式（ストレージエンジン）を選択できます。

- **Bluge (デフォルト)**: 全文検索インデックスを作成し、高速なフリーワード検索や正規表現検索を可能にします。
- **Apache Parquet**: 列指向フォーマット。大規模ログでも極めて高い圧縮率と、数値集計・分析レポートの超高速化を実現します。
- **Badger**: 高速なLSMツリー型キーバリューストア。大量ログの書き込み・スキャンに優れています。
- **Bbolt**: 安定した組み込みKey-Valueストア。

## 読み込めるファイルの種類

基本的にはタイムスタンプのあるテキスト形式のログファイルに対応しています。
タイムスタンプの形式は自動で認識できます。gz, bz2, xz, zstdで圧縮されたファイルも自動で解凍して読み込みます。
ZIPファイルやtar.gzの場合には、複数階層のフォルダーと多重圧縮に対応してます。
