...
機械学習・ディープラーニングにより、検索対象のログから通常と異なる振る舞いを示す異常ログを自動検知します。

- **Isolation Forest**: 決定木をランダム分割して外れ値を高速に分離（デフォルト）。
- **LOF (Local Outlier Factor)**: 局所的なデータ密度から外れ値を判定。
- **AutoEncoder (オートエンコーダー)**: ニューラルネットワークの再構成誤差を利用したディープラーニング異常検知。
- **LSTM (Long Short-Term Memory)**: 時系列の周期・前後関係を学習し、予測との乖離から異常を検出。
- **k近傍法 (k-NN)**: 近傍データ点との距離による密度ベース判定。
- **マハラノビス距離 (Mahalanobis Distance)**: 変数間の共分散・相関を考慮した多変量外れ値検知。
- **Z-Score / 基本統計**: 標準偏差をベースとした統計的外れ値検知。

検知しない以外を選択した場合に、特徴量の計算方法（数値データ、文字列TF-IDF、SQL/OSコマンドキーワード、曜日・時間帯など）を選択できます。

## AI正規表現クエリモード (v2.1.0)

検索条件の入力欄で「AI正規表現」モードを利用すると、「特定のIP以外からのアクセス」「レスポンスコードが500番台」「ログイン失敗」などの日本語の指示から、AI（LLM）が最適な正規表現クエリを自動作成してくれます。
検索実行時には、サイバー猫のアニメーションとともに進行状況モーダルが表示されます。
