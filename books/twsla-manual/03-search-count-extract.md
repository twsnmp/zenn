...
### 便利なオプション (v1.9.0〜v2.1.0)

- **Sixelによるターミナル内チャート表示 (`--sixel`)**:
  対応しているターミナル（iTerm2, WezTerm, mlterm等）において、グラフ画像を別ウィンドウで開くことなくターミナル画面内に直接グラフィカルに描画します。
- **抽出データの統計集計 (`--stats`)**:
  `extract` コマンド実行時に `--stats` オプションを指定することで、抽出した数値データの平均値・中央値・標準偏差・最小値・最大値を即座にターミナルへ一覧出力します。
- **IP情報（GeoIP / DNS）の自動付加**:
  抽出されたIPアドレスから自動的に逆引きホスト名や地理的位置情報（国・地域・都市名）を判定して表示できます。
